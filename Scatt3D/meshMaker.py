...
class MeshInfo():
    ...
    def __init__(self,
        ...
        if(h<0): ## pick from the below options, which should be fairly fine (mesh will be more refined near ports, PEC, and dielectric structures). Can likely go slightly larger than these.
            if(order==3): ## these numbers are semi-arbitrary, for now
        ...
        if(self.antenna_type == 'patch' or self.antenna_type == 'patchtest' or (self.antenna_type.startswith('patch_') and self.antenna_type.endswith('percentsmaller') )): ## specify the dimensions here, for a patch active near 10 GHz. Taken from FEKO sim.
        ...
        if self.comm.rank == self.model_rank: ## make all the definitions through the master-rank process
            ...
            if(self.antenna_type == 'waveguide'):
                ...
                for n in range(self.N_antennas): ## make each antenna, and prepare its surfaces to either be PEC or be the excitation surface
                    ...
                    gmsh.model.occ.rotate(itemsToRotate, 0, 0, 0, 0, 0, 1, pi/2) ## rotate to face y-hat
                    ...
                    antennaMatDimTags.append((3, patchholder))
            ...
            for boundary in gmsh.model.occ.getEntities(dim=self.fdim):
                ...
                for GP in PECSurfaceFaces: ## any surface with this point is assumed to be PEC
                    dist = gmsh.model.occ.getDistance(boundary[0], boundary[1], 2, GP)[0]
                    if(dist<distTol):
                        PEC_surfaces.append(boundary[1])
            ...
            for GP in PECSurfaceFaces:
                gmsh.model.occ.remove([(2, GP)]) ## clean-up... that shouldn't be needed
